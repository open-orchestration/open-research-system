...
# 1. Free-text topic, empty root => creates .research/docs/01-columnar-storage and returns it.
d="$(resolve_topic_dir "$TMP" "columnar storage")" \
  || { echo "MISS: returned nonzero on create"; fail=1; }
[ "$d" = "$TMP/.research/docs/01-columnar-storage" ] \
...
[ "$d3" = "$TMP/.research/docs/02-vectorized-execution" ] \
