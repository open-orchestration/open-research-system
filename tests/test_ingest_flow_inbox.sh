...
# resolve_topic_dir matches an EXISTING docs/NN-* dir, so the topic must pre-exist.
mkdir -p "$TMP/.research" "$TMP/docs/06-rag-retrieval"
...
mkdir -p "$TMP2/.research" "$TMP2/docs/06-rag-retrieval" "$TMP2/ingest"
printf 'clean default-inbox text %.0s' {1..40} > "$TMP2/ingest/cdef67890-d.md"
bash "$ROOT/scripts/ingest_flow.sh" 06-rag-retrieval
def_landed="$(find "$TMP2/docs/06-rag-retrieval/sources" -name '*.md' 2>/dev/null | wc -l | tr -d ' ')"
rm -rf "$TMP2"
