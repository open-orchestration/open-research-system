...
mkroot(){ local t; t="$(mktemp -d)"; mkdir -p "$t/.research/docs/06-rag-retrieval/sources"; echo "$t"; }
...
# Case 2: unknown topic is auto-created (resolve_topic_dir creates on miss)
TMP="$(mkroot)"
out="$(env REPO_ROOT="$TMP" DRY_RUN=1 bash "$ROOT/scripts/gather.sh" 99-nope "q" 2>&1)"; rc=$?
{ [ "$rc" -eq 0 ]; } && ok "auto-creates topic dir on unknown topic" || no "should auto-create unknown topic (rc=$rc out=$out)"
rm -rf "$TMP"
