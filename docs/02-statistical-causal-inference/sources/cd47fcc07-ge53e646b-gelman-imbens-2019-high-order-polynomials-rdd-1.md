...
|     |            |     |             |               | 2 Aug  | 2014     |         |      |               |
Abstract
Itiscommoninregressiondiscontinuityanalysistocontrolforhighorder(third,fourth,
...
notusethem,andinsteaduseestimatorsbasedonlocallinearorquadraticpolynomials
...
interpretedasthedifferencebetweenaweightedaverageoftheoutcomesforthetreated
...
∗WethankJenniferHillandJosephCumminsforhelpfulcommentsandtheNationalScienceFoundation
...
ment on some outcome. Let (Y (0),Y (1)) denoted the pair of potential outcomes for unit i,
|     |     |     |     | i   | i   |     |     |     |     |     |
| --- | --- | --- | --- | --- | --- | --- | --- | --- | --- | --- |
Yobs
and let W ∈ {0,1} denote the treatment. The realized outcome is = Y (W ). Although
|     | i   |     |     |     |     |     |     |     | i   | i i |
| --- | --- | --- | --- | --- | --- | --- | --- | --- | --- | --- |
...
discontinuity in the conditional expectation of Yobs as a function of the forcing variable, at
i
the threshold:
|     |     |     |     | limE[Y |      | x]−limE[Y |     |     |       |     |
| --- | --- | --- | --- | ------ | ---- | --------- | --- | --- | ----- | --- |
...
E[Yobs|X = x,x > 0] and µ (x) = E[Yobs|X = x,x < 0] can be well approximated by a
| i   | i   | −   | i i |     |     |
| --- | --- | --- | --- | --- | --- |
...
|     |     | i   | i +j | +i  |     |
k=0
separately on the units with values for X ≥ 0 and units with values for X < 0. The
|     |     |     | i   |     | i   |
| --- | --- | --- | --- | --- | --- |
...
imation. In that case researchers discard the units with X more than some bandwidth h
i
...
for µ and µ that can be written as weighted averages. Focusing on µˆ , the estimator for
| +   | −   |     |     |     | +   |
| --- | --- | --- | --- | --- | --- |
µ , we can write µˆ as a weighted average of outcomes for units with X ≥ 0:
| +   | +   |     |     |     | i   |
| --- | --- | --- | --- | --- | --- |
1 (cid:88)
,Yobs,
...
+
i:Xi≥0
where the weights ω have been normalized to have a mean of 1 over all units with a value
i
of the forcing variable exceeding zero. The weights are an estimator-specific function of the
3

full set of values X ,...,X for the forcing variable that does not depend on the outcome
|        |                |     | 1     | N   |     |           |         |     |     |     |     |
| ------ | -------------- | --- | ----- | --- | --- | --------- | ------- | --- | --- | --- | --- |
| values | Yobs,...,Yobs. |     | Hence | we  | can | write the | weights | as  |     |     |     |
|        | 1              | N   |       |     |     |           |         |     |     |     |     |
|        |                |     |       |     | ω   | = ω(X     | ,...,X  | ).  |     |     |     |
|        |                |     |       |     |     | i         | 1       | N   |     |     |     |
...
we estimate a K-th order polynomial approximation using all units with X less than the
i
bandwidth h (where h can be ∞ so that this includes global polynomial regressions). Then
(cid:80)
the weight for unit i in the estimation of µ , µˆ = ω Yobs/N , is
|     |     |        |            |     |          | +   | +   | i:Xi≥0 | i i  | +     |       |
| --- | --- | ------ | ---------- | --- | -------- | --- | --- | ------ | ---- | ----- | ----- |
|     |     |        |            |    |          |    |     |        | XK   | −1 |      |
|     |     |        |            |     |          | 1   | X   | ...    |      |       | 1     |
...
|     |     |     |     |     |     | j   | j   |     | j   |     | i   |
| --- | --- | --- | --- | --- | --- | --- | --- | --- | --- | --- | --- |
where e is the K-component column vector with all elements other than the first equal to
K1
...
globalpolynomialsuptosixthdegree. Nextweconsiderlocallinearmethods. Thebandwidth
...
individuals with the largest value for the forcing variable, X = 168. Because this extreme
i
value is outside the bandwidth, the weight for the local linear regression for individuals with
such a value for X would be 0. The weights for all the points included in the regression
i
...
|     |        | 5          |     |             | −3.7     |                 |          |
|     |        | 6          |     |             |          | 1.3             |          |
For different orders of the global polynomial: Normalized weight
Table 1:
...
more attractive: most of the weight goes to the individuals with values for X close to the
i
| threshold, | and individuals |     | with X | > 27.6 have | weights | of 0. |     |
| ---------- | --------------- | --- | ------ | ----------- | ------- | ----- | --- |
i
Table1alsoshowstheunattractivenessofthehighorderpolynomialregressions. Whereas
one would like to give little or zero weight to the individuals with extreme values for X , the
i
...
regressiondiscontinuitysettingscanbemisleading, inthesensethatthatconfidenceintervals
...
ThesedatawerepreviouslyusedfordifferentpurposesinworkbyLalonde(1986)andDehejia
and Wahba (1999). We look at the conditional expectation of earnings in 1978 (the outcome
Y ) given the average of earnings in 1974 and 1975 (with the predictor X divided by 10 for
| i   |     |     |     |     |     |     |     | i   |
| --- | --- | --- | --- | --- | --- | --- | --- | --- |
...
estimatorarealinearapproximationandasixthorderpolynomialapproximation. Thelinear
...
Allestimatesareinfactreasonablyclosetozero,withthenominal95%confidenceinterval
...
5,000 times we randomly pick a single point from the empirical distribution of X that will
i
...
thisrandomlydrawnvalueofX isthethresholdinaregressiondiscontinuitydesignanalysis.
i
...
1Out of the sample of 15,992 individuals there are 2044 individuals with values for X less than 1 and
...
confidenceintervalsbasedonlinearityarelikelytoincludethetruevalueofzerolessthan95%
...
| 2In 50,000 | replications | the | median | bandwidth | was 5.7. |     |     |     |
| ---------- | ------------ | --- | ------ | --------- | -------- | --- | --- | --- |
10
...
Hahn, J., Todd, P., andVanDerKlaauw, W.(2001). Regressiondiscontinuity, Econometrica
69, 201–209.
...
Jacob,B.,andLefgren,L.(2004). Remedialeducationandstudentachievement: Aregression-
...
| ------------ | ---- | -------- | --- |
Porter, J.(2003). Estimationintheregressiondiscontinuitymodel. WorkingPaper, Harvard
| University | Department | of Economics. |     |
| ---------- | ---------- | ------------- | --- |
Thistlewaite, D., andCampbell, D.(1960). Regression-discontinuityanalysis: Analternative
